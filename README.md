...
* [`src/wordle_front_end/`](src/wordle_front_end/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/wordle_front_end/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `wordle-front-end` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/wordle_front_end/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns wordle-front-end.example
  (:require [wordle-front-end.config :as config])
